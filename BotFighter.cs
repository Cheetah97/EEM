...
namespace EEM
{
    public sealed class BotFighter : BotBase
    {
        ...
        {
            DamageModule = new BotDamageHandler(this, DamageHandler);
            DiplomacyModule = new BotDiplomacyHandler(this);
        }
        ...
        public override void Main()
        {
            ...
            List<Ingame.MyDetectedEntityInfo> enemiesInProximity = RadarModule.LookForEnemies(_fighterSetup.SeekDistance, _fighterSetup.AttackNeutrals);
        ...
        void DamageHandler(IMyPlayer trash)
        {
            if (_damaged) return;
            _damaged = true;
        ...
        /*protected override void DamageHandler(IMySlimBlock block, MyDamageInformation damage)
        {
            if (_damaged) return;
            _damaged = true;

            IMyPlayer damager;
            ReactOnDamage(block, damage, PeaceDelay, out damager);
        ...
        private Ingame.MyDetectedEntityInfo GetTopPriorityTarget(List<Ingame.MyDetectedEntityInfo> targets)
        {
            ...
            if (targets.Any(x => RadarModule.Distance(x) <= 200 && RelSpeed(x) <= 40, out mostDangerous))
                return mostDangerous.OrderBy(RadarModule.Distance).First();

            List<Ingame.MyDetectedEntityInfo> targetsClose = targets.Where(x => RadarModule.Distance(x) <= 1200).ToList();
            if (targetsClose.Count > 0) return targetsClose.OrderBy(x => DangerIndex(x) / x.GetMassT()).First();
            List<Ingame.MyDetectedEntityInfo> targetsFar = targets.Where(x => RadarModule.Distance(x) > 1200).ToList();
        ...
        private float DangerIndex(Ingame.MyDetectedEntityInfo enemy)
        {
            if (enemy.Type == Ingame.MyDetectedEntityType.CharacterHuman)
                return RadarModule.Distance(enemy) < 100 ? 100 : 10;
        ...
        private void DelayedAI_Main()
        {
            try
            {
                ...
                try
                {
                    List<Ingame.MyDetectedEntityInfo> enemiesInProximity = RadarModule.LookForEnemies(_fighterSetup.AiActivationDistance);
        ...
        {
            if (BotTypeHelper.ReadBotType(RemoteControl) != BotType) return false;
